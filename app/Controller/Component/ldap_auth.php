...
class LdapAuthComponent extends AuthComponent {
		...
		if ($loginAction == $url) {
			...
			if ($isValid) {
				...
				$password = $controller->data[$this->userModel][$this->fields['password']];
                                $user = array('uid' => $username, 'password' => $password);
	...
	function login($user = null){
		...
		//$dn = $this->getDn('uid', $uid);
                $dn = $this->getDn('samaccountname', $user['uid']);
		$loginResult = $this->ldapauth($dn, $user['password']);
		if( $loginResult == 1){
			$this->_loggedIn = true;
			$user = $this->ldap->find('all', array('scope'=>'base', 'targetDn'=>$dn));
			$user_data = $user[0][$this->userModel];
			$user_data['bindDN'] = $dn;
			$user_data['bindPasswd'] = $user['password'];
