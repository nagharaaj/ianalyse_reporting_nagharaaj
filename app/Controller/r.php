...
//                        $objWorksheet = $objPHPExcel->getActiveSheet();
                        ...
                        if ($this->Auth->user('role') != 'Viewer') {
                                if($exportFormat != 'csv') {
                                        $objPHPExcel->getActiveSheet()->getStyle("A1:U1")->applyFromArray(array("font" => array( "bold" => true, 'size'  => 12, 'name'  => 'Calibri'), 'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS, 'wrap' => true), 'borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN))));
                                }
                        } else {
                        ...
                        if ($this->Auth->user('role') != 'Viewer') {
                                if($exportFormat != 'csv') {
                                        $objPHPExcel->getActiveSheet()->getStyle('L1:U1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('FCD5B4');
                                }
                        } else {
                        ...
                        if ($this->Auth->user('role') == 'Viewer') {
                                ...
                                $col = 17; 
                                $no_year= $this->ClientActualRevenueByYear->find('list', array('fields' => array('id', 'ClientActualRevenueByYear.fin_year'),'order' => 'ClientActualRevenueByYear.fin_year desc', 'group' => 'ClientActualRevenueByYear.fin_year'));
                                foreach($no_year as $no_years)
                                {
                                        $colName = PHPExcel_Cell::stringFromColumnIndex($col); 
                                        $objPHPExcel->getActiveSheet()->SetCellValue($colName.'1', 'iP'.$no_years.' Actual Revenue');
                                        $col++;
                                }
                                $colName = PHPExcel_Cell::stringFromColumnIndex($col); 
                        ...
                        foreach($arrData as $data) {
                                ...
                                if ($this->Auth->user('role') != 'Viewer') {
                                 ...
                                 if ($this->Auth->user('role') != 'Viewer'|| $this->Auth->user('role') !='Country - Viewer') {
                                        ...
                                        if($exportRevenue == "YES") {
                                                ...
                                                if(isset($prevRevenue($recordId))){
                                                        foreach($prevRevenue[$recordId] as $year => $revenueValue)
                                                        {
                                                           $row[] = $revenueValue;
                                                        }
                                                } else {
